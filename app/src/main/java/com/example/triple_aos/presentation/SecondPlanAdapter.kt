...
class SecondPlanAdapter : RecyclerView.Adapter<SecondPlanAdapter.NewPlanViewHolder>() {
    ...
    override fun onCreateViewHolder(
        parent: ViewGroup,
        viewType: Int
    ): SecondPlanAdapter.NewPlanViewHolder {
        val binding =
            LayoutAddPlanBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return SecondPlanAdapter.NewPlanViewHolder(binding)
    }

    override fun onBindViewHolder(holder: SecondPlanAdapter.NewPlanViewHolder, position: Int) {
        entire_plans?.let {
